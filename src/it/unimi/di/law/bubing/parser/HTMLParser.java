...
public class HTMLParser<T> implements Parser<T> {
    ...
    public byte[] parse(final URI uri, final HttpResponse httpResponse, final LinkReceiver linkReceiver) throws IOException {
        ...
        @SuppressWarnings("resource") final StreamedSource streamedSource = new StreamedSource(new InputStreamReader(contentStream, charset));
            ...
            if (segment.getEnd() > lastSegmentEnd) {
                ...
                if (segment instanceof StartTag) {
                    ...
                    if (name == HTMLElementName.IFRAME || name == HTMLElementName.FRAME || name == HTMLElementName.EMBED)
                        ...
                        if (returnNoFollow || !"nofollow".equalsIgnoreCase(startTag.getAttributeValue("rel")))
                            process(linkReceiver, base, startTag.getAttributeValue("href"));
                    } else if (name == HTMLElementName.AREA || name == HTMLElementName.LINK)
                        process(linkReceiver, base, startTag.getAttributeValue("href"));
        ...
        if (DigestAppendable.DEBUG)
            if (digestAppendable != null) {
                LOGGER.info("Closing " + digestAppendable.debugFile + " for " + uri);
    ...
    public static void main(final String arg[]) throws IllegalArgumentException, IOException, URISyntaxException, JSAPException, NoSuchAlgorithmException {
        ...
        LOGGER.info("DigestHexString: " + Hex.encodeHexString(digest));
        LOGGER.info("Links: " + linkReceiver.urls);
        ...
        if (urlStrings.size() != linkReceiver.urls.size()) {
            LOGGER.info("There are " + linkReceiver.urls.size() + " URIs but " + urlStrings.size() + " strings");
        }
